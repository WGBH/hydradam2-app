...
describe Asset, :requires_fedora do
  ...
  let(:xml_file_containing_pbcore) { XMLFile.new.tap { |pcdm_file| pcdm_file.content = Fixtures.read('pbcore/valid_pbcore_1.xml') } }

  describe "#assign_properties_from_pbcore_file" do
    let(:pbcore_fixture_titles) { ["Nova", "Gratuitous Explosions", "3-2-1", "Kaboom!"] }
    ...
    context "when it has an associated PBCore file" do
      ...
      it "assigns properties from an uploaded PBCore file" do
        pbcore_fixture_titles.each do |pbcore_fixture_title|
